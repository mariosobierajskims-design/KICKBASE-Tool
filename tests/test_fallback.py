...
def test_all_five_played_uses_all_five():
    ...
    assert form.start_rate == 1.0


def test_four_of_five_played_uses_those_four():
    entries_desc = [
        entry(10, None, played=False, started=False),
        entry(9, 4),
        entry(8, 6),
        entry(7, 2),
        entry(6, 10),
...
def test_three_of_five_played_falls_back_to_last_four_actually_played():
    entries_desc = [
        entry(12, None, played=False, started=False),
        entry(11, 5),
        entry(10, None, played=False, started=False),
        entry(9, 7),
        entry(8, 3),
...
def test_two_of_five_played_falls_back_to_last_five_actually_played():
    entries_desc = [
        ...
        entry(11, 5),
        entry(10, None, played=False, started=False),
        entry(9, None, played=False, started=False),
        entry(8, 3),
        entry(7, 9),
        entry(6, 1),
        entry(5, 4),
...
def test_zero_played_uses_last_five_ever_played_regardless_of_age():
    entries_desc = [
        entry(20, None, played=False, started=False),
        entry(19, None, played=False, started=False),
        entry(18, None, played=False, started=False),
        entry(17, None, played=False, started=False),
        entry(16, None, played=False, started=False),
        entry(5, 6),
        entry(4, 8),
        entry(3, 2),
        entry(2, 4),
...
def test_start_rate_counts_only_starts_within_chosen_games():
    entries_desc = [
        entry(5, 8, started=True),
        entry(4, 4, started=False),  # substitute appearance
        entry(3, 6, started=True),
        entry(2, 2, started=True),
        entry(1, 10, started=False),
    ]
    form = compute_recent_form(entries_desc)
    assert form.games_used == 5
    assert form.start_rate == 3 / 5


def test_no_games_played_ever_returns_none_values():
    entries_desc = [entry(1, None, played=False, started=False)]
    form = compute_recent_form(entries_desc)
    ...
    assert form.start_rate is None
...
def test_fewer_career_games_than_needed_degrades_gracefully():
    ...
    entries_desc = [
        entry(5, None, played=False, started=False),
        entry(4, None, played=False, started=False),
        entry(3, None, played=False, started=False),
        entry(2, 7),
        entry(1, 3),
    ]
    form = compute_recent_form(entries_desc)
